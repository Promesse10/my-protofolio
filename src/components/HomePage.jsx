...
import gif from "../IMAGE/unscreen.gif"
...
export default function Portfolio() {
  ...
  const startTutorialCycle = () => {
    setShowTutorial(true)
    setIsFading(false)

    // Start fading out after 17 seconds
    setTimeout(() => {
      setIsFading(true)
    }, 17000)

    // Hide tutorial and reset cycle after 20 seconds
    setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const initialTimer = setTimeout(startTutorialCycle, 500)

    // Set up the recurring cycle every 20 seconds
    timerRef.current = setInterval(startTutorialCycle, 20000)
      ...
      <aside className={`w-64 bg-slate-800 p-6 fixed h-full overflow-y-auto transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 z-40`}>
        <div className="flex flex-col items-center mb-8">
          <div className="relative w-24 h-24">
            <div className="absolute inset-0 rounded-full bg-blue-300 opacity-25 animate-ping z-0" />
            <img
              src={picture}
              alt="Profile"
              className="w-full h-full rounded-full relative z-10 cursor-pointer"
              onClick={handleClick}
            />
            {showTutorial && (
              <div
                className={`absolute left-2/3 transform -translate-x-1/2 -translate-y-1/2 z-20 animate-bounce transition-opacity duration-3000 ${isFading ? 'opacity-0' : 'opacity-100'
                  }`}
              >
            ...
            {showGift && (
              <div className={`absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 z-30 transition-opacity duration-1000 ${isFading ? 'opacity-0' : 'opacity-100'}`}>
                <img
                  src={gif}
                  alt="Gift"
